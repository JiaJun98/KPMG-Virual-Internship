...
def data_frame_creater(file,spreadsheet):
    ...
    header=df.iloc[0]
    edited_df=df[1:]
    edited_df.rename(columns=header,inplace=True) 
    return edited_df
...
#Remove column "default" cause the numbers seemed irrelevant
...
#Converted all incorrect data types(datetime to strings)
